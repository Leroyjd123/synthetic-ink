...
const PoemList: React.FC<PoemListProps> = ({ poems, type, onSelect, onDelete }) => {
  if (poems.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-slate-400 dark:text-slate-600 animate-fade-in">
        ...
        >
          ...
          {onDelete && (
            <button
              ...
              className="absolute top-3 right-3 p-1.5 text-slate-300 dark:text-slate-600 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30 dark:hover:text-rose-400 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
              title="Remove"
            >
